...
export default class Triangle implements GeometryDriver {
  ...
  private static pickThirdPointFromBaseAndHeight(
    pA: Point,
    pB: Point,
    height: number,
    toTheLeft: boolean,
    ...
    const base = new Line(pA, pB);
    const mid = pA.midPoint(pB);
    const h = base.normalizedDirection;
    const perp = new Point(-h.y, h.x);
    const signed = toTheLeft ? 1 : -1;
    return mid.add(perp.multiply(height * signed)).rename(name ?? "");
  }
   ...
   * @param {coordinatesTriangleArray} coordinatesTriangle is an array of 2 points with 2d coordinates: [[number, number], [number, number]]
  ...
  /**
   * fromSides builds a triangle where pA is in origin (0,0) and pB at (c,0)
   * pC is at the intersection of circles of radius a (centered at pA) and radius b (centered at pB)
   * @param {number} a is the side of the triangle opposite of pA
   * @param {number} b is the side of the triangle opposite of pB
   * @param {number} c is the side of the triangle opposite of pC
   * @param {string} name is the optional name of the triangle
   * @throws TypeError if a,b, or c are not finite numbers
   * @throws RangeError if a,b, and c are not valid triangle sides
   * @returns {Triangle} a new Triangle at coordinates pA:[0,0] and pB:[c,0] and pC calculated
   */
  static fromSides(a: number, b: number, c: number, name?: string): Triangle {
    if (!Number.isFinite(a) || !Number.isFinite(b) || !Number.isFinite(c)) {
      throw new TypeError("Triangle.fromSides expects finite numbers");
    }
    if (!Triangle.isValidTriangleSides(a, b, c)) {
      throw new RangeError("Invalid triangle side lengths");
    }

    const pA = new Point(0, 0);
    const pB = new Point(c, 0);
    const x = (b * b + c * c - a * a) / (2 * c);
    const y2 = b * b - x * x;
    if (y2 < -EPSILON) {
      throw new RangeError("No real triangle for these sides");
    }
    const pC = new Point(x, Math.sqrt(Math.max(0, y2)));
    return new Triangle(pA, pB, pC, name);
  }

  /**
   * Creates a triangle given the base length and two angles.
   *
   * @param base - The length of the base of the triangle. Must be positive.
   * @param angleA - The first angle of the triangle in degrees.
   * @param angleB - The second angle of the triangle in degrees.
   * @param name - Optional name for the triangle.
   * @returns A new Triangle instance.
   * @throws {RangeError} If the base is not positive or if the angles do not form a valid triangle.
   */
  static fromBaseAndAngles(
    base: number,
    angleA: Angle,
    angleB: Angle,
    name?: string,
    ...
    const C = Math.PI - A - B;
    if (C <= EPSILON) {
      throw new RangeError("Angles do not form a valid triangle");
    }

    const c = base;
    const a = c * Math.sin(A) / Math.sin(C);
    const b = c * Math.sin(B) / Math.sin(C);
    return Triangle.fromSides(a, b, c, name);
  }

  static fromBaseAndHeight(
    pA: Point,
    pB: Point,
    height: number,
    toTheLeft: boolean = true,
    name?: string,
  ): Triangle {
    ...
    if (pA.isSameLocation(pB)) {
      throw new RangeError("Base points must be distinct");
    }

    const base = new Line(pA, pB);
    const mid = pA.midPoint(pB);
    const dir = base.normalizedDirection;
    const perp = new Point(-dir.y, dir.x);
    const sign = toTheLeft ? 1 : -1;
    const pC = mid.add(perp.multiply(height * sign));
    return new Triangle(pA, pB, pC, name);
  }

  static fromTwoPointsAndSide(
    pA: Point,
    pB: Point,
    sideAC: number,
    toTheLeft: boolean = true,
    ...
    if (!Number.isFinite(sideAC) || sideAC <= 0) {
      throw new RangeError("sideAC must be positive");
    }
    if (pA.isSameLocation(pB)) {
      throw new RangeError("Base points must be distinct");
    }
    ...
    const x = (d * d + sideAC * sideAC - sideAC * sideAC) / (2 * d);
    const heightSq = sideAC * sideAC - x * x;
    if (heightSq < -EPSILON) {
      throw new RangeError("Impossible geometry for the given constraints");
    }

    const mid = pA.add(dir.multiply(x));
    const sign = toTheLeft ? 1 : -1;
    const pC = mid.add(perp.multiply(Math.sqrt(Math.max(0, heightSq)) * sign));
    return new Triangle(pA, pB, pC, name);
  }
