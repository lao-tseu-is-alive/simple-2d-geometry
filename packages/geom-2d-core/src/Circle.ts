...
export type ReadonlyCircle = Omit<Circle, 'center' | 'radius' | 'name' | 'translate' | 'rotate' | 'rotateAround'> & {
...
export default class Circle implements GeometryDriver {
    ...
    set center(input: Readonly<iPoint>) {
        assertIsIPoint(input, "Circle center")
        this._center = new Point(input.x, input.y, input.name);
    }
    ...
    sameLocation(other: Circle): boolean {
        ...
        return (this._center.isSameLocation(other._center) && (Math.abs(this._radius - other._radius) < EPSILON));
    }
    ...
    private getPointOnCircle(angleRadians: number): Point {
        const dx = this._radius * Math.cos(angleRadians);
        const dy = this._radius * Math.sin(angleRadians);
        return new Point(
            this._center.x + dx,
            this._center.y + dy
        );
    ...
    containsPoint(other: Readonly<iPoint>): boolean {
        ...
        return distanceSq <= Math.pow(this._radius, 2);
    }
    ...
    distanceToPointSq(other: Readonly<iPoint>): number {
        assertIsIPoint(other, "distanceToPointSq other")
        const dx = this._center.x - other.x;
        const dy = this._center.y - other.y;
        return dx * dx + dy * dy;
    }
    ...
    intersectsCircle(other: Circle): boolean {
        ...
        const distanceSq = this.distanceToPointSq(other._center);
    ...
    translate(other: Readonly<iPoint>): Circle {
        assertIsIPoint(other, "Circle translate other")
        const newCenter = this._center.add(other);
        return new Circle(newCenter, this._radius, this._name)
    }
    ...
    rotate(theta: Angle): Circle {
        if (this._center.isOrigin()) {
            return this.clone()
        }
        return new Circle(this._center.rotate(theta), this._radius, this._name)
    }
    ...
    rotateAround(theta: Angle, center: Readonly<iPoint>): Circle {
        assertIsIPoint(center, "Circle rotateAround center")
        // if given center is origin no need translate
        if (center.x === 0 && center.y === 0) {
            return this.rotate(theta);
        }
        // Translate points so center is the origin
        const centerTranslated = this._center.subtract(center);
        const centerRotated = centerTranslated.rotate(theta);
        // Translate point back
        return new Circle(centerRotated.add(center), this._radius, this._name);
    }
