import Point, {assertIsPoint, assertIsIPoint, type coordinate2dArray, type iPoint, type ReadonlyPoint} from "./Point.ts";
...
export type ReadonlyPolygon = Omit<Polygon, 'points' | 'name' | 'translate' | 'rotate' | 'rotateAround' | 'scale' | 'scaleAround' | 'setVertex'> & {
...
export default class Polygon implements GeometryDriver {
     ...
     * @param {ReadonlyArray<iPoint>} input is the new array of vertices
     */
    set points(input: ReadonlyArray<iPoint>) {
        if (!Array.isArray(input) || input.length < 3) {
            throw new RangeError("Polygon needs at least 3 vertices");
        }
        for (const p of input) {
            assertIsIPoint(p, "Polygon points");
        }
        this._points = input.map(p => new Point(p.x, p.y, p.name));
    }
    ...
    constructor(points: ReadonlyArray<iPoint>, name?: string) {
        ...
        for (const p of points) {
            assertIsIPoint(p, "Polygon constructor");
        }
        this._points = points.map(p => new Point(p.x, p.y, p.name));
     ...
     */
    clone(): Polygon {
        return new Polygon(this._points, this._name);
    }
     ...
     * @returns {Polygon} return this instance of the object (to allow function chaining)
     */
    setVertex(index: number, point: Readonly<iPoint>): this {
        assertIsIPoint(point, "Polygon setVertex");
        if (index < 0 || index >= this._points.length) {
            throw new RangeError("Index out of bounds");
        }
        this._points[index] = new Point(point.x, point.y, point.name);
        return this;
    }
     ...
     */
    containsPoint(point: Readonly<iPoint>): boolean {
        assertIsIPoint(point, "Polygon containsPoint");
        let inside = false;
    ...
    translate(other: Readonly<iPoint>): Polygon {
        assertIsIPoint(other, "Polygon translate other");
        return new Polygon(
    ...
    rotateAround(theta: Angle, center: Readonly<iPoint>): Polygon {
        assertIsIPoint(center, "Polygon rotateAround center");
        return new Polygon(
            this._points.map(p => p.rotateAround(theta, new Point(center.x, center.y, center.name))),
            this._name,
        );
    }
    ...
    scaleAround(factor: number, center: Readonly<iPoint>): Polygon {
        assertIsIPoint(center, "Polygon scaleAround center");
        const centerPt = new Point(center.x, center.y, center.name);
        return new Polygon(
            this._points.map(p => p.subtract(centerPt).multiply(factor).add(centerPt)),
            this._name,
        );
    }
